...
class MuZeroConfig:
    def __init__(self):
        ...
        ### Game
        self.num_agents = 5
        self.observation_shape = (1, 1, 49)  # Dimensions of the game observation, must be 3D (channel, height, width). For a 1D array, please reshape it to (1, 1, length of array)
        ...
        self.max_moves = 6000  # Maximum number of moves if game is not finished before
...
class GFootball:
    def __init__(self):
        self._env = football_env.create_environment(env_name="5_vs_5",
                                                    stacked=False,
                                                    representation='raw',
                                                    ...
                                                    render=False)
        self.N = 5          # N: num_agents in a team
        self.observation_size = 8 * self.N + 9  # 49
        self.state = None
    ...
    def step(self, action):
        self.state, reward, done, _ = self._env.step(action)

        if not done:
            reward = np.mean(reward)  # 1: score / -1: opponent score / 0
        else:
            reward = np.sign(self.state[0]['score'][0] - self.state[0]['score'][1]) * 20
